...
def fetch_reviews(app_id, lang, country, sort, count):
    ...
    while fetched_count < count:
        try:
            batch_count = min(500, count - fetched_count)
            ...
            if not current_reviews:
                logging.info("Tidak ada ulasan lagi yang ditemukan.")
                break
            ...
            logging.error(f"Terjadi kesalahan saat mengambil ulasan: {e}")
            break
...
def save_reviews_to_csv(reviews_list, directory, filename):
    ...
    try:
        ...
        logging.error(f"Terjadi kesalahan saat menulis ke file CSV: {e}")
